...
#------------------------------------------------------------------------------------------------
# Biidrectional LSTM with padding
#------------------------------------------------------------------------------------------------
class BiPadLSTMModel(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, pooling=False, name='BiPadLSTM'):
        super().__init__()
        self.name = name
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.pooling = pooling
    ...
    def forward(self, x):
        embedded = self.embedding(x)
        # lunghezze sequenze ignorando padding
        ...
        if self.pooling:
            ...
            output = output * mask
            feat = output.sum(dim=1) / lengths.unsqueeze(1)
        else:
            h_forward = hn[0]
            h_backward = hn[1]
...
class BiLSTMModel(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, pooling=False, name='BiLSTM'):
        super().__init__()
        self.name = name
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.pooling = pooling
        ...
        # hidden_dim * 2 perché forward + backward
        self.classifier = nn.Linear(hidden_dim * 2, 1)
    ...
    def forward(self, x):
        ...
        # hn: (2, batch, hidden_dim)

        if self.pooling:
            # mean pooling sulle features bidirezionali
            feat = output.mean(dim=1)
        else:
            # concateniamo forward e backward hidden state finali
            h_forward = hn[0]
            h_backward = hn[1]
            feat = torch.cat((h_forward, h_backward), dim=1)
